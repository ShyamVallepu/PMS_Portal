...
export class PensionDetailsComponent implements OnInit {
  ...
  handlePensionerInput() {
    this.pensionerInput.pensionType=Number(this.pensionerInput.pensionType)
    this.pensionerInput.bankType=Number(this.pensionerInput.bankType)
    console.log(this.pensionerInput);

    this.pservice.addPensionDetails(this.pensionerInput)
      .subscribe(
        data => {
      ...
      /*this.http.post('https://localhost:44391/api/PensionerDetail/Add',this.pensionerInput,{responseType: 'text'}).subscribe(data=>{
        console.log(data)
      }
